...
public:
    ...
    // json 专用查询 (ts 在 item["timestamp_ns"])
    std::vector<const T*> query_json(uint64_t after, size_t limit) const {
        std::vector<const T*> out;
        for (auto& item : buffer_) {
            uint64_t ts = item.value("timestamp_ns", 0ULL);
            if (ts > after) out.push_back(&item);
        }
        size_t n = limit ? std::min(limit, out.size()) : out.size();
        return {out.end() - n, out.end()};
    }
